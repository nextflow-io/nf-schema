...
4. Removed the deprecated `validation.failUnrecognisedParams` and `validation.failUnrecognisedHeaders` configuration options

## Changes

1. Improved the overall efficiency of the plugin. This should be especially noticeable for larger samplesheets.
...
1. Added three new optional arguments to the `paramsSummaryLog()` function, these take precedence over the configuration options:
...
3. Values of the [`MemoryUnit`](https://www.nextflow.io/docs/latest/reference/stdlib-types.html#memoryunit) type are now considered to be integers of bytes instead of a string
4. Values of the [`Duration`](https://www.nextflow.io/docs/latest/reference/stdlib-types.html#duration) type are now considered to be integers of milliseconds instead of a string
5. Only `null` values are now excluded from the parameter validation instead of all values that resolve to `false`. This means that empty strings, empty lists and `false` values will now be validated instead of being excluded from validation. The reasoning behind this change is to make the validation more reliable for all inputs

## Bug fixes

1. Fixed an issue where `Path` objects generated by the parameters using the new static types would cause a `StackOverflowError` during parameter validation.
2. Support for allowed Nextflow types in parameters now works correctly.
3. Cast parameters to the expected types when using the strict configuration syntax. This should fix issues with parameters being validated as strings when they were expected to be a different type.
